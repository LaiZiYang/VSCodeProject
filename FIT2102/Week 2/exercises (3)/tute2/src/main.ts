...
const prettifyTodoItem = (item: TodoItem): string => {
  return ["UserId: " + item.userId, "Id: " + item.id, "Title: " + item.title, "Completed: " + item.completed].join(" | ")
}
...
const prettifyContent = (content: ReadonlyArray<TodoItem>): string =>
  content.map(e=>prettifyTodoItem(e)).join("\n");
...
const prettifyTodo = (todo: Todo): string => {
  return ["TimeStamp: " + todo.timestamp, "From: " + todo.from, "Content: \n" + prettifyContent(todo.content)].join("\n")
};

outputText("pretty_object_output", prettifyTodo(todo));
 ...
 */
function prettifyBinaryTree<T extends Object>(node: BinaryTree<T>, indent: string = "|-"): string {
  const current = node.data.toString();

  const left = node.left != undefined ? indent + prettifyBinaryTree(node.left, indent + "|-") : null;
  const right = node.right != undefined ? indent + prettifyBinaryTree(node.right, indent + "|-") : null;
 ...
 */
function prettifyNaryTree<T extends Object>(node: NaryTree<T>, indent: string = "|-"): string {
  const current = node.data.toString();

  const children = node.children ? node.children.map(e=>indent + prettifyNaryTree(e, indent + "|-")) : [];
